...
public interface MainDao extends JpaRepository<Main, Long> {
	@Query("SELECT m FROM Main m WHERE m.shopId = :shopId and m.inputDate = :inputDate and m.orderDate = null")
	public List<Main> findByShopIdInputdate(@Param("shopId") String shopId, @Param("inputDate") Date inputDate);
